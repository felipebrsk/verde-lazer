            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    <form class="border px-4 pt-2 pb-3" method="POST" action="{{ route('profile-update', $profile->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="role" class="col-form-label">Posição</label>
                            <select name="role" class="form-control">
                                <option value="">-----Selecionar posição-----</option>
                                <option value="admin" {{ $profile->role == 'admin' ? 'selected' : '' }}>Administrador
                                </option>
                                <option value="user" {{ $profile->role == 'user' ? 'selected' : '' }}>Usuário</option>
                            </select>
                            @error('role')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
